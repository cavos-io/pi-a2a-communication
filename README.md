...
`PI_A2A_PORT` starts the A2A server for that process. You can also set `PI_A2A_SERVER_ENABLED=true` explicitly. Use `PI_A2A_AGENT_ID` and `PI_A2A_AGENT_NAME` to give each local agent a stable identity, `PI_A2A_ADVERTISED_URL` to override the Agent Card URL, `PI_A2A_BASE_PATH` to serve A2A endpoints under a path prefix, and `PI_A2A_AUTH=none` for unauthenticated local discovery.
